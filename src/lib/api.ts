...
export async function createRazorpayOrder(orderId:string):Promise<{razorpayOrderId:string;amount:number;currency:string;keyId:string;orderId:string;orderNumber:string}>{return apiRequest('/api/payments/razorpay/order',{method:'POST',body:JSON.stringify({orderId})});}
export async function verifyRazorpayPayment(input:{orderId:string;razorpayOrderId:string;razorpayPaymentId:string;razorpaySignature:string}):Promise<Order>{return normalize(await apiRequest<Order>('/api/payments/razorpay/verify',{method:'POST',body:JSON.stringify(input)}));}
