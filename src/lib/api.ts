...
export interface CreateOrderInput { userId?:string|null; customerName:string; customerPhone:string; customerEmail:string; deliveryAddress:string; deliveryLatitude?:number|null; deliveryLongitude?:number|null; orderNotes:string; paymentMethod:'cod'|'razorpay'; items:CartItem[]; couponCode?:string; }
...
export interface RazorpayCheckoutInput { userId?:string|null; customerName:string; customerPhone:string; customerEmail:string; deliveryAddress:string; deliveryLatitude?:number|null; deliveryLongitude?:number|null; orderNotes:string; items:CartItem[]; couponCode?:string; }
export async function createRazorpayCheckout(input:RazorpayCheckoutInput):Promise<{razorpayOrderId:string;amount:number;currency:string;keyId:string;orderNumber:string}>{return apiRequest('/api/payments/razorpay/checkout',{method:'POST',body:JSON.stringify({userId:input.userId||null,customerName:input.customerName,customerPhone:input.customerPhone,customerEmail:input.customerEmail,deliveryAddress:input.deliveryAddress,deliveryLatitude:input.deliveryLatitude??null,deliveryLongitude:input.deliveryLongitude??null,orderNotes:input.orderNotes,items:input.items.map(i=>({menuItemId:i.menuItem.id||i.menuItem._id,name:i.menuItem.name,quantity:i.quantity,imageUrl:i.menuItem.imageUrl})),couponCode:input.couponCode||''})});}
