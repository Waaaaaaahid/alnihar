...
export async function sendNewOrderNotifications(order: OrderLike) {
  await Promise.allSettled([sendOneSignal(order), sendWhatsApp(order)]);
}
...
async function sendOneSignal(order: OrderLike) {
  ...
  const response = await fetch('https://api.onesignal.com/notifications', {
    ...
    body: JSON.stringify({
      app_id: appId,
      target_channel: 'push',
      include_aliases: { external_id: [env('ONESIGNAL_ADMIN_EXTERNAL_ID') || 'admin'] },
...
async function sendWhatsApp(order: OrderLike) {
  ...
  const response = await fetch(`https://graph.facebook.com/v23.0/${phoneNumberId}/messages`, {
    ...
    body: JSON.stringify({
      ...
      type: 'template',
      template: {
        name: env('WHATSAPP_ORDER_TEMPLATE') || 'new_order_alert',
        language: { code: env('WHATSAPP_TEMPLATE_LANGUAGE') || 'en_US' },
        components: [{ type: 'body', parameters: [
          { type: 'text', text: order.orderNumber },
          { type: 'text', text: order.customerName },
          { type: 'text', text: `₹${Number(order.total).toFixed(0)}` },
          { type: 'text', text: order.paymentMethod.toUpperCase() },
          { type: 'text', text: order.deliveryAddress || order.orderType },
        ] }],
      },
    }),
  });
